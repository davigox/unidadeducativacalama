...
const Tareas = (props) => {
    ...
    const obtenerCalificaciones = () => {
        db.collection('tareasCursos').doc(props.idCurso).collection('tareas').where('idTrimestre', '==', props.idTrimestre).where('idUsuario', '==', props.idUsuario).get().then(querySnapshot => {
            const notas = []
            let sumatoria = 0
