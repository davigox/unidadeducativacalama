...
const Notas = (props) => {
    ...
    const obtenerCalificaciones = () => {
        db.collection('notasCursos').doc(props.idCurso).collection('notas').where('idTrimestre', '==', props.idTrimestre).where('idUsuario', '==', props.idUsuario).get().then(querySnapshot => {
            const notas = []
            let sumatoria = 0
