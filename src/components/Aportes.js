...
const Aportes = (props) => {
    ...
    const obtenerCalificaciones = () => {
        db.collection('aportesCursos').doc(props.idCurso).collection('aportes').where('idTrimestre', '==', props.idTrimestre).where('idUsuario', '==', props.idUsuario).get().then(querySnapshot => {
            const notas = []
            let sumatoria = 0
