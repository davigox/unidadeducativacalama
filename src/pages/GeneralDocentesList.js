...
const GeneralDocentesList = (props) => {
    ...
    const buscarEstudiantes = async (curso) => {
        await db.collection('users').orderBy('apellidoPaterno').where('rol', '==', 'docente').get().then((querySnapshot) => {
            const docs = [];
...
export default GeneralDocentesList
